...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        na = len(self.actions.keys()) # self.actions = {right:(),left:(),down:(),up:()} self.actions_list={right,left,down,up}
        discount = self.gamma
        lr = self.alpha
        # initialization
        self.allowed_movements()  # self.allowed_moves = {"state_index":[1,0,3,2]}
    ...
    def get_policy(self, Q): # Policy : tell agent what action to take in a certain state
...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ...
        # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
        state_action_table = np.zeros((ns, na))
        while episode <= self.episode_max:
            s_current = init_pos
            R_total = 0
            steps = 0
            while not end_episode:
                ...
                action = None
                possible_actions_index = np.random.randint(low=0,high=len(possible_actions),size=1)
                # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
                action_index = possible_actions[possible_actions_index[0]]
                action_str = self.action_list[action_index]
                msg = {"action": action_str, "exploration": True}
                self.sender(msg)

                state_action_table[steps][action_index] = state_action_table[s_current][action_index] + 1
        ...
        n = np.max(state_action_table,axis=1)

        return n

    def get_policy(self, Q): 
        # Policy : tell agent what action to take in a certain state
        # The utility of a Q state -> Q(s,a) : You already make a action from s state, but it leads to which states is stll
        # unknown, so Q(s,a) = sum(T(s,a,s')*[R(s,a,s')+V(s')])
        # Q = [ns]
        print(" q size")
        print(Q.shape)
