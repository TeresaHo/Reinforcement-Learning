...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        r = np.random.rand(1)
        if r[0]<epsilon:
            possible_actions_index = np.random.randint(low=0,high=len(all_actions),size=1)
            action = all_actions[possible_actions_index[0]]
        else:
        ...
        # It is recommended you use the np.random module
        SL = ScheduleLinear(anneal_timesteps, epsilon_final)
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
                Q[s_current][action] = (1-lr) * Q_old[s_current][action] + lr * ( R + discount * np.nanmax(Q_old[s_next]) ) 
            ...
            rv_nan_Q = Q[np.logical_not(np.isnan(Q))]
            rv_nan_Q_old = Q_old[np.logical_not(np.isnan(Q_old))]
            diff = np.sum(np.abs(rv_nan_Q-rv_nan_Q_old))/len(rv_nan_Q)
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        new_p = self.initial_p + delta * (t/self.schedule_timesteps)
        return new_p
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
